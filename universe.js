...
// Explicitly scroll on every activation, including when the hash is already set.
const wangzaiCard=document.querySelector('#carousel .work-card[href="#wangzai"]');
const wangzaiSection=document.getElementById('wangzai');
if(wangzaiCard&&wangzaiSection){
  wangzaiCard.addEventListener('click',event=>{
    if(event.ctrlKey||event.metaKey||event.shiftKey||event.altKey)return;
    event.preventDefault();
    if(location.hash!=='#wangzai')location.hash='wangzai';
    wangzaiSection.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'start'});
  });
}
...
document.querySelectorAll('.time-card').forEach((card,index)=>{
  ...
  button.setAttribute('aria-label','展开或收起'+heading.textContent);
  button.setAttribute('aria-expanded','false');
  paragraph.hidden=true;card.classList.add('is-collapsed');card.classList.remove('is-selected');
